  ...
  const handleSendPhoneOtp = async () => {
    ...
    try {
      const response = await fetch('/api/send-otp', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ type: 'whatsapp', contact: phone, otp })
      });
      ...
      showToast('OTP sent to your WhatsApp successfully!', 'success');
    } catch (error: any) {
      console.error(error);
      showToast(`Error: ${error.message || 'sending WhatsApp OTP'}`, 'error');
    }
  };
              ...
              <div className="flex flex-col gap-2">
                <div className="flex gap-2">
                  ...
                  {role === 'doctor' && !isPhoneVerified && (
                    <button type="button" onClick={handleSendPhoneOtp} className="px-4 py-3 bg-[#25D366] text-white font-semibold rounded-xl whitespace-nowrap hover:bg-[#128C7E] flex items-center gap-2">
                      ...
                      {phoneOtpSent ? 'Resend' : 'Send WhatsApp OTP'}
                    </button>
                  )}
                  {role === 'doctor' && isPhoneVerified && (
                    <span className="px-4 py-3 bg-[#25D366]/20 text-[#128C7E] font-semibold rounded-xl flex items-center whitespace-nowrap">✓ WhatsApp Verified</span>
                  )}
                </div>
